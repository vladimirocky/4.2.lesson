...
Webpack splits all JavaScript files and those that not should.
For example, when is used `css-loader` Webpack will split runtime API scripts of the loader, that used by compiling of CSS modules into CSS file.
Webpack can't differentiate what is really user's JavaScript files and what is runtime scripts of modules.
Such "appetite" of Webpack completely breaks the compilation process.

**You must concrete specify what scripts should be splitted**, using `optimization.splitChunks.cacheGroups`:
...
Note: in the `test` option must be specified all extensions of scripts which should be splitted.
